...
inline Eigen::VectorXd get_G(const std::vector<double>& qs){
	...
	G(0,0) = g*(m1*t2*2.8024836E-1+m2*t2*2.8024836E-1+m3*t2*2.8024836E-1-Pcx0*m0*t6-Pcy0*m0*t2-Pcz1*m1*t2-Pcx2*m2*t2*t4+Pcx1*m1*t6*t7+Pcy1*m1*t3*t6+Pcy2*m2*t2*t8-Pcz2*m2*t3*t6+Pcz3*m3*t2*t8-Pcx3*m3*t2*t4*t5+Pcx3*m3*t3*t6*t9-Pcx2*m2*t6*t7*t8+Pcy3*m3*t3*t5*t6-Pcy2*m2*t4*t6*t7+Pcy3*m3*t2*t4*t9-Pcz3*m3*t4*t6*t7-Pcx3*m3*t5*t6*t7*t8+Pcy3*m3*t6*t7*t8*t9);
